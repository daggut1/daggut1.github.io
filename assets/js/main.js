$(document).ready(function() {
  ...
  function friendlyGreeting() {
    const welcomeBannerHero = $('#welcome-banner');
    const welcomeBannerPortfolio = $('#welcome-banner-portfolio');

    // Function to update and display the banner
    const updateBanner = (bannerElement) => {
      if (!bannerElement.length) return;

      const lastVisitCookie = document.cookie.split('; ').find(row => row.startsWith('lastVisit='));
      let message = 'Welcome to my homepage for the first time!';
      let isNewUser = true;

      if (lastVisitCookie) {
        const lastVisitDate = new Date(decodeURIComponent(lastVisitCookie.split('=')[1]));
        message = `Welcome back! Your last visit was ${lastVisitDate.toLocaleString()}`;
        isNewUser = false;
      }

      bannerElement.text(message);
      bannerElement.addClass('show'); // Add class to trigger animation

      // Set cookie for 1 year expiry
      document.cookie = `lastVisit=${encodeURIComponent(new Date().toISOString())};path=/;max-age=31536000`;
    };

    // Update banners in both sections
    updateBanner(welcomeBannerHero);
    updateBanner(welcomeBannerPortfolio);
    ...
    // Remove the timeout to make the banner visible indefinitely
    // if (isNewUser) {
    //   setTimeout(() => {
  ...
  // --- Cookie-based Welcome Greeting as Alert ---
  ...
  // --- Show/Hide Email ---
  // This ensures the code runs only after the entire page is loaded and ready.
  // The outer $(document).ready(function() { ... }); already handles this.
  // So, we can directly place the logic here.

  // Step 1: Select the HTML elements and store them in variables
  var $email = $('#email');      // Selects the span with id="email"
  var $toggle = $('#toggle-email'); // Selects the button with id="toggle-email"
  
  // Step 2: Store the original email address in a variable for later use
  var emailValue = $email.text();
  
  // Step 3: Create a "state" variable to track if the email is currently hidden or shown
  var hidden = false; // Initially, the email is not hidden
  
  // Step 4: Attach a click event listener to the button
  // This function will run every time the button is clicked
  $toggle.on('click', function() {
  
    // Step 5: The Toggling Logic
    if (!hidden) {
      // --- THIS RUNS IF THE EMAIL IS CURRENTLY VISIBLE ---
      
      // Change the span's text to asterisks
      $email.text('••••••••••••••••••••••');
      
      // Change the button's text to "Show"
      $toggle.text('Show');
      
      // Update the aria-pressed attribute for screen readers (good for accessibility)
      $toggle.attr('aria-pressed', 'true');
      
      // Update our state variable to remember that the email is now hidden
      hidden = true;
      
    } else {
      // --- THIS RUNS IF THE EMAIL IS CURRENTLY HIDDEN ---
      
      // Change the span's text back to the original email (from the variable we saved)
      $email.text(emailValue);
      
      // Change the button's text back to "Hide"
      $toggle.text('Hide');
      
      // Update the aria-pressed attribute
      $toggle.attr('aria-pressed', 'false');
      
      // Update our state variable to remember that the email is now visible
      hidden = false;
    }
  });
